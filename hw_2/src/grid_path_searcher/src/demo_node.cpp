...
{   
    ...
    map_vis.header.frame_id = "world";  //map_vis.header.frame_id = "/world";
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    _astar_path_finder->AstarGraphSearch(start_pt, target_pt);  //A* search，新建一个文件
    //Astar_searcher.h里面有AstarGraphSearch函数的头文件和实现文件
    //具体在Astar_searcher.cpp中实现

    //Retrieve the path
    auto grid_path     = _astar_path_finder->getPath();//将搜索到
    auto visited_nodes = _astar_path_finder->getVisitedNodes();

    //Visualize the result
    visGridPath (grid_path, false);  //false -> Do not use JPS
